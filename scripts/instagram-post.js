...
function pickNext(images, posted) {
  ...
  const lastFilename = posted[posted.length - 1];
...
async function main() {
  ...
  if (!cap) {
    console.log(`No caption found for ${next.filename} — skipping.`);
    queue.posted.push(next.filename);
    fs.writeFileSync(queuePath, JSON.stringify(queue, null, 2));
    return;
  }

  const hashtags  = generateHashtags(cap.title, cap.caption, next.category);
  ...
  console.log(`Posting:   ${next.filename}`);
